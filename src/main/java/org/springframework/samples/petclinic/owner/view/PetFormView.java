...
public abstract class PetFormView extends VerticalLayout {
    ...
    PetFormView(PetFormPresenter<? extends PetFormView> presenter) {
        ...
        binder.forField(nameTextField).asRequired().bind(PetFormDto::getPetName, PetFormDto::setPetName);

        DatePicker birthDatePicker = new DatePicker();
        binder.forField(birthDatePicker).asRequired().bind(PetFormDto::getPetBirthDate, PetFormDto::setPetBirthDate);

        ComboBox<PetType> typeComboBox = new ComboBox<>();
        typeComboBox.setItems(presenter.findPetTypes());
        binder.forField(typeComboBox).asRequired().bind(PetFormDto::getPetType, PetFormDto::setPetType);
