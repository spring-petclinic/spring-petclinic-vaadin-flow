...
import org.springframework.samples.petclinic.visit.view.VisitCreateView;
...
public class OwnerDetailsView extends VerticalLayout implements BeforeEnterObserver {
    ...
    OwnerDetailsView(OwnerDetailsPresenter presenter) {
        ...
        editOwnerButton
                .addClickListener(
                        e -> UI.getCurrent().navigate(OwnerEditView.class, new RouteParameters(OWNER_ID_ROUTE_PARAM, presenter.getOwner().getId().toString())));

        Button addNewPetButton = new Button(getTranslation("addNewPet"));
        addNewPetButton
                .addClickListener(
                        e -> UI.getCurrent().navigate(PetCreateView.class, new RouteParameters(OWNER_ID_ROUTE_PARAM, presenter.getOwner().getId().toString())));
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        final Integer ownerId = event.getRouteParameters().getInteger(OWNER_ID_ROUTE_PARAM).orElse(null);
    ...
    class PetsContainer extends AbstractCompositeField<VerticalLayout, PetsContainer, List<Pet>> {
        ...
        private void addPet(Pet pet) {
            ...
            petBinder.bind(petBirthDateTextField, p -> DateTimeFormatter.ofPattern("yyyy-MM-dd").format(p.getBirthDate()), null);
            ...
            petVisitsGrid.addColumn(new LocalDateRenderer<>(Visit::getDate, DateTimeFormatter.ofPattern("yyyy-MM-dd")))
            ...
            editPetButton.addClickListener(e -> {
                RouteParam ownerIdParam = new RouteParam(OWNER_ID_ROUTE_PARAM, presenter.getOwner().getId().toString());
                RouteParam petIdParam = new RouteParam("petId", pet.getId().toString());
                UI.getCurrent().navigate(PetEditView.class, new RouteParameters(ownerIdParam, petIdParam));
            });
            Button addVisitButton = new Button(getTranslation("addVisit"));
            addVisitButton.addClickListener(e -> {
                RouteParam ownerIdParam = new RouteParam(OWNER_ID_ROUTE_PARAM, presenter.getOwner().getId().toString());
                RouteParam petIdParam = new RouteParam("petId", pet.getId().toString());
                UI.getCurrent().navigate(VisitCreateView.class, new RouteParameters(ownerIdParam, petIdParam));
            });
