...
import org.springframework.samples.petclinic.owner.OwnerDetailsView;
import org.springframework.samples.petclinic.system.MainContentLayout;
import org.springframework.samples.petclinic.view.FormUtil;
...
@Route(value = "owners/:ownerId?([0-9]+)/pets/:petId?([0-9]+)/visits/new", layout = MainContentLayout.class)
public class VisitCreateView extends VerticalLayout implements BeforeEnterObserver {
    ...
    VisitCreateView(VisitCreatePresenter presenter) {
        ...
        binder.forField(birthDateTextField).bind(v -> DateTimeFormatter.ofPattern("yyy-MM-dd").format(v.getPetBirthDate()), null);
        ...
        final FormLayout petForm = new FormLayout(nameTextField, birthDateTextField, typeTextField, ownerTextField);
        petForm.setResponsiveSteps(new ResponsiveStep("0", 1, LabelsPosition.ASIDE), new ResponsiveStep("40em", 4, LabelsPosition.TOP));

        final DatePicker visitDatePicker = new DatePicker();
        binder.forField(visitDatePicker).asRequired().bind(VisitCreateDto::getVisitDate, VisitCreateDto::setVisitDate);

        final TextField descriptionTextField = new TextField();
        descriptionTextField.setWidthFull();
        binder.forField(descriptionTextField).asRequired().bind(VisitCreateDto::getDescription, VisitCreateDto::setDescription);
        ...
        previousVisitsGrid.addColumn(Visit::getDescription).setHeader(getTranslation("description"));

        final VerticalLayout previousVisitsContainer = new VerticalLayout(new H3(getTranslation("previousVisits")), previousVisitsGrid);

        add(new H2(getTranslation("newVisit")), new H3(getTranslation("pet")), petForm, visitForm, addVisitButton, previousVisitsContainer);
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        final Integer ownerId = event.getRouteParameters().getInteger("ownerId").orElse(null);
    ...
    public void navigateToOwnerDetails(Integer ownerId) {
        UI.getCurrent().navigate(OwnerDetailsView.class, new RouteParameters("ownerId", ownerId.toString()));
    }
